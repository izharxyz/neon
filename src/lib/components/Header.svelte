	...
	<div class="flex-none gap-2">
		<form class={showSearch ? 'form-control' : 'hidden form-control'} action="/search/{search}">
			<input
				type="text"
				placeholder="Search"
				class="input input-bordered input-secondary w-20 md:w-64 max-w-xs"
				bind:value={search}
			/>
		</form>
		{#if ready}
			...
			<button class="btn dropdown dropdown-end z-50">
				...
				<ul
					class="dropdown-content menu p-2 shadow bg-base-100 rounded-box mt-4 border border-primary-content"
				>
			...
			<div class="dropdown dropdown-end">
				...
				>
					<form method="POST">
						...
						<li>
							<button formaction="/auth/logout" type="submit" class="normal-case">Logout</button>
						</li>
					</form>
				</ul>
			</div>
